    ...
    <div className="px-3 py-2.5 flex items-center justify-between gap-2">
      ...
      <div className="text-right flex items-center gap-1">
        ...
        <span
          ...
          style={{ color: isZero ? '#64748b' : isPositive ? color : '#4ade80' }}
        >
...
function MiniStat({
  ...
  return (
    <div
      className="rounded-lg p-2.5 bg-slate-900/60 border border-slate-800"
    ...
    >
      ...
      {count > 0 ? (
        <>
          ...
          <p className="text-[9px] text-slate-600 mt-0.5">ha · {count.toLocaleString('id-ID')} fitur</p>
        </>
      ) : (
        <p className="text-sm text-slate-700 mt-0.5 font-medium">—</p>
      )}
    </div>
  );
}
